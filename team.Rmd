...
plot2 <-
  ggplot(defCleanFilter,
  aes(x = `West Virginia`, y = `Kansas State`, na.rm = TRUE)) +
  geom_jitter(width = 0.1, height = 0.1) + xlim(0, 10) + ylim(0, 10) + geom_abline(slope =
  1, intercept = 0)
...
A greater number of points lie above the abline (x=y), indicating that the defense played better in their game against Kansas State than against West Virginia becauseo of the increase increase in the amount of tackles.
...
```{r}
offReceive <-
  group_by(offBio, Name) %>% summarise(
  R_YDS_mean = mean(Receiving_YDS),
  R_YDS_std = sd(Receiving_YDS),
  Weight = mean(Weight)
  )

offReceive

ggplot(offBio, aes(x = reorder(Name, Weight, FUN = mean), y = Receiving_YDS)) + geom_boxplot() + theme(axis.text.x =
  element_text(angle = 90, hjust = 1))+ xlab("Name In Order of Weight")+ylab("Receiving Yards")+ ggtitle("Receiving Yards By Player in Order of Weight")

ggplot(offBio, aes(x = Weight, y = Receiving_YDS)) + geom_point()+ ggtitle("Receiving Yards by Weight")
```
<br />

No obvious relationship exists between weight and receiving yards.
Heavier player did seem to recieve more in short distances but this did not hold up for long distances. 
...
Q: Which position had the most solo tackles?
<br />
A: The defensive back position had the most solo tackles. This could possibly be due to their positionign on the line which gives them the ability to see more of what is going on. 